...
solutionPoints:
  ...
  - title: "Multiple detection engines. "
    detail: "Cloudflare combines deterministic heuristics, machine learning, client-side signals, and verified-bot validation. Enterprise Bot Management adds domain-specific analysis and exposes JA3/JA4 fingerprints, detection IDs, signed-agent status, and other fields for precise policies."
  - title: "Continuous session verification with Precursor. "
    detail: "Available across all plans, Precursor repeatedly evaluates browser behavior instead of relying on one request or challenge. It updates session state in the cf_clearance cookie, contributes context to bot scores, and can re-verify a visitor when behavior changes. Choose Minimize Friction for background verification or Maximize Security to require a valid session, then use Precursor Rules to apply modes by path or hostname."
...
faq:
  ...
  - question: "How is Precursor different from JavaScript Detections and Challenges?"
    answer: "JavaScript Detections performs a one-time browser check, while Precursor continuously evaluates behavior across the session and supersedes JavaScript Detections when enabled. Challenges remain point-in-time verification events; Precursor uses session context to determine when a visitor should be re-verified. Existing JavaScript Detection-driven Security Rules continue to apply after enabling Precursor."
...
diveDeeper:
  docs:
    ...
    - title: "Precursor session-based detection"
      url: "https://developers.cloudflare.com/cloudflare-challenges/precursor/"
